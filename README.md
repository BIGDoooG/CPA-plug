...
当前正式版本：`0.6.0`，平台：`linux/amd64`。

`0.6.0` 使用 CPA 原生只读额度接口进行 Codex 安全观察，不发送合成模型请求；
探测失败不会新增、延长或升级凭证+模型冷却，模型恢复以真实业务请求成功为准。

`0.5.4` 的历史版本修复了冷却管理页展示问题；当前 `0.6.0` 进一步修复主动探测误判和冷却状态回退问题。每模型最低保护通道默认值为 `5`。

当前发布包以 0.5.3 的 Codex 调度器为基线，保留已有的并发、错误分类和凭证+模型冷却能力；Claude 账号不会进入 Codex OAuth 额度刷新或 Codex 主动探测链路。
